using Microsoft.EntityFrameworkCore.Storage;
using SDV.BorderControl.API.Core.Entities;
using SDV.BorderControl.API.Core.Interfaces;
using SDV.BorderControl.API.Infrastructure.Data;

namespace SDV.BorderControl.API.Infrastructure.Repositories;

public class UnitOfWork : IUnitOfWork
{
    private readonly ApplicationDbContext _context;
    private IDbContextTransaction? _transaction;

    public UnitOfWork(ApplicationDbContext context)
    {
        _context = context;
        Missions = new Repository<Mission>(_context);
        Vehicles = new Repository<Vehicle>(_context);
        Alerts = new Repository<Alert>(_context);
        TelemetryRecords = new Repository<TelemetryRecord>(_context);
    }

    public IRepository<Mission> Missions { get; }
    public IRepository<Vehicle> Vehicles { get; }
    public IRepository<Alert> Alerts { get; }
    public IRepository<TelemetryRecord> TelemetryRecords { get; }

    public async Task<int> SaveChangesAsync()
    {
        return await _context.SaveChangesAsync();
    }

    public async Task BeginTransactionAsync()
    {
        _transaction = await _context.Database.BeginTransactionAsync();
    }

    public async Task CommitTransactionAsync()
    {
        if (_transaction != null)
        {
            await _transaction.CommitAsync();
            await _transaction.DisposeAsync();
            _transaction = null;
        }
    }

    public async Task RollbackTransactionAsync()
    {
        if (_transaction != null)
        {
            await _transaction.RollbackAsync();
            await _transaction.DisposeAsync();
            _transaction = null;
        }
    }

    public void Dispose()
    {
        _transaction?.Dispose();
        _context.Dispose();
    }
}
